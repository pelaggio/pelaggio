...
- `npx pelaggio doc-review <path>` is the read-only review path (provider-diverse, report bound to the document sha256). `npx pelaggio pr-review --pr <n>` is **effectful** — it upserts a PR comment and posts the required commit status, which CI normally owns — and `npx pelaggio land --pr <n>` **merges** (squash + branch delete behind the red-merge guard). Both need authorization separate from a request to review; an agent asked only to review must use `doc-review` and must not post statuses or land.
