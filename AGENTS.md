...
- `docs/agent-context/backlog-intake-audit.md`: (instrumentation, complete) the 198-item open backlog audited against the `/charter` probes and the `charter-contract.md` grammar, frozen + fingerprinted (`node --import tsx ci/backlog-audit.ts`, corpus `423:ced7d044cf7a`) — six items sit outside the `autopilot` pickup label and are invisible to `/pick`, three of them (#70/#154/#155) naming gaps still live in the tree; **TC-017 and TC-018 are both double-booked** between a `guarantee` registry claim and an unbuilt control in an open issue. Empty charters are a July stock, not a live rate (35% → 9%), so the answer is refinement, not an intake gate (ADR-0011 bars one). Plus the nulls that bear on #530/#531 and the closed-dependency sweep that already pays for #296.
