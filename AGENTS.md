...
- (flow, planned) Flow events live in `.dev/flow-events.jsonl` (separate from the cycle-log, shared envelope + reader); event `type` is namespaced (`pelaggio.*` closed/core-validated, consumer events vendor-prefixed + schema-registered); the reader is tolerant-with-diagnostic and back-compatibly reads untyped legacy cycle records.
