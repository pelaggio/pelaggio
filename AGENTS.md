...
- Every blocking gate clears the gate-quality bar (`guarded-actions.md` §8.1): **early** (fires before work exists), **narrow** (refuses the violating dimension, never the work; delta-scoped, not absolute-state), **preserving** (a refusal checkpoints/parks/leaves state intact and names the resume path — refusal ≠ discard), **falsifiable** (no-false-fire tests ship with the gate). The bar governs how gates behave, never whether to add more.
