...
- The Claude custom spawn is unconditional and fail-closed: every Claude SDK child starts through `spawnClaudeSeat` (Linux Bubblewrap PID + mount namespace, or an explicit macOS Seatbelt preview, plus a detached terminal session). Do not bypass it, omit `--new-session` / detached Seatbelt spawn, gate it on `onChildSpawn`, or conflate it with Landlock / the full contained-execution jail.
