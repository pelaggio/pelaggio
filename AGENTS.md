...
- (flow, planned) The landing queue is target-agnostic and defers to the provider's merge queue in PR mode; pelaggio owns integration ordering only for `direct-push`, where the exclusive-access primitive is **git ref compare-and-swap** in the harness (verification bound to the candidate SHA; explicit `--force-with-lease`). `bd merge-slot` is an **optional ordering layer above** that fence, never a replacement, gated on a positive typed-output probe; when used, the slot lives in one shared `MAIN_REPO/.beads` and pelaggio owns ordering + waiter-hygiene + dead-holder reconcile. See ADR-0025.
