...
- PR candidate blockers may be removed only by a complete, valid isolated verification report; verifier failure retains them. Under #495 carry, a prior SHA's complete report may be the removing authority for an untouched-path refutation — never a fresh model shortcut.
...
- (guards, proposed — `guarded-actions.md` §8.2, pending an ADR-0026 amendment) A guard whose guarantee has a *completeness surface* — many sites, paths, or inputs where the property could be violated — is established **by construction** at one chokepoint, not patched site-by-site (`guarded-actions.md` §8.2): recurrence of "one more" instance of the *same class* (one more output sink, one more untriaged env var, one more merge/HEAD shape) is the signal to hoist. The moves: **chokepoint enforcement** (put the guarantee where every path funnels — e.g. one publish funnel for review output, the #615 prerequisite, not each render site), **extract-and-require** (a conformance test fails on a new untriaged member instead of leaking it), **default-deny allowlist** (trust is a small explicit set; unknown/future members denied), **recognize-by-invariant** (identify a thing by its defining property, not its incidental shape). Triggered by recurrence of a class, never applied prophylactically; a chokepoint the codebase cannot force all paths through is a false guarantee (#435) — name and test that all paths funnel through it.
...
- (flow, planned) Flow events live under `.dev/flow-events/` as one append-only segment per writer execution context (single-writer-per-file, no shared-file concurrent append; separate from the cycle-log, shared envelope + reader); event `type` is namespaced (`pelaggio.*` closed/core-validated, consumer events vendor-prefixed + schema-registered); the reader is tolerant-with-diagnostic and back-compatibly reads untyped legacy cycle records.
...
Each invariant above is a one-line index; the full rationale lives in the routed detail docs below. Invariants tagged `(flow, planned)` are target-state — see `docs/agent-context/flow.md`; the tag drops when the implementing item ships. `(guards, proposed)` marks a construction rule whose authoritative form (an ADR-0026 amendment) has not been made; its intent half is already graph intent (`CON-0027`).
...
- `docs/agent-context/charter-contract.md`: (design) the charter contract — acceptance anchors (`AC-n` + `verify:` bindings) and assumption ledger (`A-n` + `wrong-if:`) as a lint-not-gate grammar; the `charter-defect` blocked kind + decorrelated-retry reframe gate completing ADR-0011's seam; supersede-based re-charter adjudication; fenced recursive decomposition; the charter-is-the-only-human-owned-artifact handoff rule.
- `docs/agent-context/throughput-economy.md`: (investigation, complete) the check-ratchet hypothesis tested and **refuted** for this repo — citation-edge spawn measurement, the two evidence defects behind the motivating anecdote; what the data does support (intake exceeds closure ~1.7× uniformly; landing cost concentrated in review and repeat cycles); why content-based gate tiering is barred by ADR-0013/0016/0017/0024; `deferred` is a park mechanism, not a graveyard.
- `docs/agent-context/backlog-triage.md`: (investigation, complete) the 2026-08-29 backlog-collapse hypothesis tested and **largely refuted** against a frozen 195-item corpus (`0167df41b7e5b826`) — both existing detectors return zero (`stale-scan` 0 hits under a 3/3 positive control; title-tier duplicate sweep 0 true duplicates); the credential-reachability cluster is six disjoint channels and an §8.2 hoist signal, not a merge; net proposal −1 of 195. Carries method findings for #530 (prospective-path false positives), #531 (title-tier misses the only real duplicate) and #519 (no metadata disposition in the closure vocabulary).
- `docs/agent-context/delivery-packet.md`: (design → ADR-0028) the claim-scoped delivery packet — unit ladder (attempt/delivery/item/campaign-as-fold/release-as-fold), the four-audience contract, intent-only dossier + identity manifest carried on the landing surface with every evaluative fact joined; attestation layer (ADR-0018/`ai-delivery`) carried never duplicated; direct-push in-candidate carriage gated on the ADR-0025 executor; §12 open-findings ledger.
- `docs/agent-context/local-autopilot.md`: (design → ADR-0029) local-autopilot contract v0 — Run as the public durable object, WorkContract input, journal-authoritative recovery, required verification evidence, explicit consent + truthful assurance for uncontained host execution, `ready_for_review` as success; CLI projections over start/get/continue/cancel.
- `docs/agent-context/review-gate-baseline.md`: (instrumentation) the landing-cost baseline from persisted gate records, frozen + fingerprinted (`npx tsx ci/review-metrics.ts --until 2026-08-25`, corpus `82:e7615e16408b`) — **1.64 re-review rolls per landing** vs a ≤1 target, 11/30 ever-pass, $277.65/passing PR, wall-clock NOT instrumented (so latency-shaped changes like #547 are invisible to it); `consensus-block` has never occurred in 82 rolls, which is ADR-0024's designed split-parking rather than drift. Plus the encoding-loss protocol and its first run — coverage gaps that produced Q14 and CON-0027 on the #616 branch, and a withdrawn fixture whose retraction is itself the method lesson.
- `docs/agent-context/corpus-convergence.md`: (RFC, nothing adopted) author governing knowledge once as data and derive every representation that can be derived — the reduced proposition / decision / evidence kernel, views-not-stores, and the phase sequencing that puts the authoring cutover first. Carries the successor corpus in `docs/agent-context/data/corpus/` as its living artifact, with a domain checker that admits a constraint only if it binds the harness, requires an assumption to carry either a counterexample or a revisit trigger, and refuses a renderer naming a node the corpus does not contain. Experiment interpretation lives in #670, never here. A settled decision belongs in a re-founded ADR-0027, not here.
- `docs/assurance/`: (shadow, non-authoritative — ADR-0027) the machine-readable intent graph: `shadow-graph.json` (propositions with invariant/constraint/assumption roles, decisions, realizations, typed relations, ADR + AGENTS.md coverage maps), `views.json` (named projections), `README.md` (ontology, grounding limits, maintenance rules), and the shadow experiments (question contract and assessment evidence here; charter normalization and activity provenance arrive with #623). Checked by `pnpm test:ci`; nothing may cite it as authority.
...
- `docs/agent-context/provider-quota.md`: (design) per-provider quota telemetry (the reported/probed/estimated channel hierarchy over each provider's native grammar) + the meter → projector → strategy-chain seam for balancing throughput vs diversity vs pool headroom; feeds #246/#465/#578.
