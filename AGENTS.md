...
- `docs/agent-context/review-gate-baseline.md`: (instrumentation) the landing-cost baseline measured from persisted gate records (`npx tsx ci/review-metrics.ts`) — 2.79 rolls/PR, 38% ever-pass, $275.92/passing PR; **`consensus-block` has never occurred in 81 rolls**, so the operating regime is a one-vote veto rather than convergence; the numbers any throughput claim must beat, plus the encoding-loss protocol (semantic gap / lossy flattening / authoring-cost gap / no-value) for testing a requirements model without grading it by its own judgment. **First run retrodicted against the #616 stack**: 5 realizations for 56 propositions, all 8 public `TC-*` claims have zero implementing realizations, `realization` is never a legal relation target, and Q10 checks realization→purpose instead of proposition→realization. The #625 fixture's premise was wrong (my truncated grep; issue closed invalid) and `ci/assurance-reachability.ts` — built for it — is what caught the error.
