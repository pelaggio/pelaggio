...
describe("checkAllowlist", () => {
	...
	it("covers every ALLOWED_PREFIX with a fixture (exhaustiveness lock)", () => {
		const fixtures = ["scripts/autopilot/main.ts", ".claude/skills/pick/SKILL.md", ".claude-templates/migration-checklist.md", "bin/claude-autopilot.js"];
	...
	it("rejects tests nested inside allowed dirs", () => {
		const files = [
			{ path: "scripts/autopilot/__tests__/x.ts", size: 100 },
			{ path: "scripts/autopilot/x.test.ts", size: 100 },
		];
		const violations = checkAllowlist(files);
