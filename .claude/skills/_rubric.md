...
- `providers/claude.ts` — owns the SDK `query()` loop, hook installation, event streaming; `providers/<name>.ts` for the other drivers; `providers/index.ts` is the registry; `step-runner.ts` only dispatches. No business logic.
- `pipeline.ts` — one cycle (`runPipeline`); `orchestrator.ts` — the loop that schedules cycles, waits out parks and drains reviews. Both compose everything above. No direct SDK imports (goes through `step-runner` → `providers/`).
...
- **Worktree isolation**: `providers/claude.ts` installs `PreToolUse` hooks when running in a worktree to block Write/Edit/Bash calls targeting `MAIN_REPO` paths. This prevents agents from corrupting sibling worktrees. The hook must run before every mutating tool — don't add exceptions without a test.
...
- **Guarantee authority**: enumerate the inputs each new or widened mechanism asserts a guarantee over (paths, environment, generated/config grammar, provider output, git/HEAD shapes, records, or other state). Resolve ownership in this order: declared write-set when present, then optional `_project-context.md` ownership map; the assurance graph is non-authoritative context only. Flag every asserted input outside that authority, naming the input and (when known) the owning item; revise the plan when this item can honestly own it, otherwise re-charter. Do **not** flag a plan or diff whose every asserted input is owned. A findings note that introduces or widens a mechanism must name the acceptance criterion it serves, otherwise it is a re-charter, not a revision.
