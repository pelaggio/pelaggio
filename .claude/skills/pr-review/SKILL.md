...
- The diff itself: `git diff origin/main...HEAD`. Do not use `gh pr diff` — the review seat holds no GitHub credentials (#554), so `gh` calls only burn turns on an auth failure.
