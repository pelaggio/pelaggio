...
2. **Pelaggio directive** — if `Arguments:` contains `pelaggio plan-review` or `pelaggio code-review`, dispatch to that mode regardless of git state.
3. **Plan-only state** — extract the item ID from the current branch name and resolve the plan path via `npx pelaggio roadmap plan-path --id <ID> --worktree "$PWD"` (exit 0 means it exists). If the plan exists AND `git diff --name-only main...HEAD` lists only that plan path (or other paths under the same `docs/plans/` or `.dev/plans/` directory), it's plan review mode — target is the resolved plan path.
...
- **Deferred** — feature extensions beyond scope, performance optimizations without evidence, refactors to untouched code, logical future extensions. Add to the roadmap (pelaggio mode) or list in the output (inline mode).
...
- **RETHINK** — fundamental design issue that editing cannot fix. In plan review, this aborts the pelaggio cycle. In code review, it signals the implementation should go back to `/plan`.
...
4. **Deferred** — items added to roadmap (pelaggio) or listed (inline)
