...
5. Emit `pick-item: <ID>` on its own line (the ID you just claimed, e.g. `pick-item: COMP-11C-II`) immediately followed by `pick-result: claimed` on the next line. The pipeline reads the ID from this marker — free-text ID mentions elsewhere in the output can be ambiguous when parent/child IDs share a prefix.
