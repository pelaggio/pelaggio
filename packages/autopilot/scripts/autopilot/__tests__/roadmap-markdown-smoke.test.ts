...
// The one test that wires the real config seam end to end: an `.autopilot.yml`
...
function seedRepo(): string {
	const dir = mkdtempSync(join(tmpdir(), "autopilot-md-smoke-"));
	tmpDirs.push(dir);
...
describe("markdown roadmap — config-selected source drives a real lifecycle", () => {
	it("`.autopilot.yml` source: markdown resolves an adapter that ships on main", async () => {
		const repo = seedRepo();
		seedFile(repo, ".autopilot.yml", "roadmap:\n  source: markdown\n");
