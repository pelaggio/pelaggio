...
describe("ensureWorktreeDeps (materialize)", () => {
	...
	it("end-to-end regression: cross-package workspace import resolves into the worktree's source", () => {
		...
		// shape: packages/web imports @cdhorne/claude-autopilot-server; the
		// MAIN-side <web>/node_modules/@cdhorne/claude-autopilot-server symlink
		...
		const { main, worktree } = makeMaterializeSetup({
			workspaces: [
				{ pkg: "packages/web", name: "@cdhorne/claude-autopilot-web" },
				{ pkg: "packages/server", name: "@cdhorne/claude-autopilot-server" },
			],
			rootMainNm: {
				workspaceLinks: [
					{ name: "@cdhorne/claude-autopilot-web", targetPkg: "packages/web" },
					{ name: "@cdhorne/claude-autopilot-server", targetPkg: "packages/server" },
				],
				pnpmInfra: true,
			},
			worktreeNm: { kind: "symlink-to-main" },
			subpackages: [{ pkg: "packages/web", mainNm: { workspaceLinks: [{ name: "@cdhorne/claude-autopilot-server", targetPkg: "packages/server" }] }, worktreeNm: { kind: "absent" } }],
		});
		...
		// The @cdhorne/claude-autopilot-server symlink under packages/web resolves
		// to the worktree's source — readable via realpathSync.
		const importPath = resolve(worktree, "packages/web/node_modules/@cdhorne/claude-autopilot-server");
