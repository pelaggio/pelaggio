...
describe("review status and comments", () => {
	...
	it("upserts the marker-bearing findings comment by REST id", () => {
		const { run, calls } = stub((args) => (args[1] === "repos/o/r/issues/9/comments" ? { stdout: JSON.stringify([{ id: 42, body: "<!-- autopilot-pr-review -->\nold", created_at: "2026-01-01T00:00:00Z" }]) } : {}));
		assert.equal(upsertReviewComment(run, "o/r", 9, "<!-- autopilot-pr-review -->\nnew"), true);
		assert.deepEqual(calls[1], ["api", "--method", "PATCH", "repos/o/r/issues/comments/42", "-f", "body=<!-- autopilot-pr-review -->\nnew"]);
	});
...
describe("prepareReviewHead", () => {
	it("fetches the PR head and creates a detached data worktree", () => {
		...
		assert.deepEqual(out, { diffCwd: join(repo, ".dev", "review-heads", "abc123"), baseRef: "origin/main", headRef: "refs/autopilot-review/pr-9" });
		assert.ok(cmds.includes("git fetch origin refs/pull/9/head:refs/autopilot-review/pr-9"));
...
describe("cleanupReviewHead", () => {
	it("removes the head worktree and deletes the fetched ref", () => {
		...
		assert.deepEqual(cmds, [`git worktree remove --force ${join(repo, ".dev", "review-heads", "abc123")}`, "git update-ref -d refs/autopilot-review/pr-9"]);
		rmSync(repo, { recursive: true, force: true });
	});
	...
	it("skips the worktree remove when the path is absent and stays fail-soft on error", () => {
		...
		assert.deepEqual(cmds, ["git update-ref -d refs/autopilot-review/pr-9"]);
		rmSync(repo, { recursive: true, force: true });
	});
});
