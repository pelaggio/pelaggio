...
Mobile-responsive web UI for the pelaggio control-plane daemon. Astro 5 + React 19 + Tailwind v4. Static output, served by the daemon at `/ui/`. Manifest-based PWA — install to home screen on iOS/Android, no service worker.
...
```bash
pnpm --filter @pelaggio/web test
pnpm --filter @pelaggio/web check:types
```
