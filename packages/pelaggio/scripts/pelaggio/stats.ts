...
export function renderDashboard(stats: Stats): string {
	...
	const header = `${A.bold("pelaggio stats")}`;
	const summary = `${stats.totalCycles} cycles  ${fmtUsd(stats.totalCostUsd, stats.costEstimated)}`;
	lines.push(`${header}${" ".repeat(Math.max(1, 60 - "pelaggio stats".length))}${A.dim(summary)}`);
	lines.push("");
...
export function runStatsCommand(opts: { json: boolean } = { json: false }): void {
	if (!existsSync(LOG_PATH)) {
		...
		console.log(A.dim("No pelaggio log found at") + " " + LOG_PATH);
		return;
	}
