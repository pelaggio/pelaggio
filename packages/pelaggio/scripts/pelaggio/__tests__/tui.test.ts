...
describe("computeTuiEnabled", () => {
	it("env override wins over isTTY", () => {
		assert.equal(computeTuiEnabled({ PELAGGIO_PLAIN: "1" }, { isTTY: true }), false);
	});
	...
	it("PELAGGIO_PLAIN=0 does not disable (only exact '1')", () => {
		assert.equal(computeTuiEnabled({ PELAGGIO_PLAIN: "0" }, { isTTY: true }), true);
	});
});
