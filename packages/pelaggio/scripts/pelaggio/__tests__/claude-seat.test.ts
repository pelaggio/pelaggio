...
const DIRECT_ANTHROPIC_CREDENTIAL_VARS = CLAUDE_CLI_AUTH_VARS.slice(0, 3);
const CLAUDE_CLI_SHARED_AUTH_VARS = CLAUDE_CLI_AUTH_VARS.slice(3);
...
function isolatedHarnessPaths(): { home: string; tmpdir: string; xdgRuntimeDir: string; xdgConfigHome: string; ghConfigDir: string; claudeConfigDir: string; anthropicConfigDir: string } {
	...
	return {
		...
		anthropicConfigDir: join(root, "anthropic"),
	};
}
...
describe("spawnClaudeSeat", () => {
	it("preserves ordinary SDK env while denying harness Git config, and reports only a positive wrapper PID", () => {
		...
		assert.deepEqual(seen[0]?.options.env, { PATH: "/usr/bin" });
		for (const name of ["GIT_CONFIG_COUNT", "GIT_CONFIG_KEY_0", "GIT_CONFIG_VALUE_0", "GIT_CONFIG_PARAMETERS", "GIT_CONFIG"]) {
			assert.equal(name in (seen[0]?.options.env ?? {}), false, `seat must drop ${name}`);
		}
...
describe("buildClaudeSeatEnv", () => {
	...
	it("preserves metadata-service credential opt-outs for every role", () => {
		const metadataOptOuts = {
			AWS_EC2_METADATA_DISABLED: "true",
			METADATA_SERVER_DETECTION: "none",
		} as const;
		const metadataSource = sdkShapedEnv({ ...metadataOptOuts });
		for (const step of ALL_STEPS) {
			const env = buildClaudeSeatEnv(metadataSource, step, []);
			for (const [name, value] of Object.entries(metadataOptOuts)) assert.equal(env[name], value, `${step} must preserve ${name}`);
		}
	});

	it("denies proxy configuration to every role until the credentialed-proxy gate lands", () => {
		// A proxy URL can carry userinfo credentials (http://user:pass@proxy), so forwarding these
		// safely requires the value-conditional gate that classifies and refuses a credentialed
		// proxy. This slice ships the env DENIAL only; granting the vars without the gate would hand
		// every seat a credential. The follow-on slice adds both together and flips this expectation
		// to a forwarding assertion. Pinned rather than left untested so the deny is a stated
		// property, not an accident of the allowlist.
		const proxyConfig = {
			HTTP_PROXY: "http://user:pass@proxy.corp:3128",
		...
		for (const step of ALL_STEPS) {
			...
			for (const name of Object.keys(proxyConfig)) assert.equal(name in env, false, `${step} must not forward ${name} without the credentialed-proxy gate`);
			assert.equal("SENTINEL_SECRET" in env, false);
		}
	});

	it("drops provider credentials for every role when no matching provider mode is selected", () => {
		// sdkShapedEnv carries live-looking provider credentials but no truthy USE_* selector.
		for (const step of ALL_STEPS) {
			const env = buildClaudeSeatEnv(source, step, allow);
			for (const name of FOUNDRY_MODE_CREDENTIAL_VARS) assert.equal(name in env, false, `${step} must drop ${name} outside Foundry mode`);
			for (const name of ANTHROPIC_AWS_MODE_CREDENTIAL_VARS) assert.equal(name in env, false, `${step} must drop ${name} outside Anthropic AWS mode`);
		...
		for (const value of ["0", "false", "", "off", "provider-config-value-1"]) {
			for (const selector of ["CLAUDE_CODE_USE_BEDROCK", "CLAUDE_CODE_USE_FOUNDRY", "CLAUDE_CODE_USE_ANTHROPIC_AWS", "CLAUDE_CODE_USE_VERTEX"]) {
				const env = buildClaudeSeatEnv(sdkShapedEnv({ [selector]: value }), "pr-review", allow);
				assert.equal("AWS_ACCESS_KEY_ID" in env, false, `${selector}=${JSON.stringify(value)} must not pass AWS credentials`);
				assert.equal("ANTHROPIC_FOUNDRY_API_KEY" in env, false, `${selector}=${JSON.stringify(value)} must not pass Foundry credentials`);
				assert.equal("ANTHROPIC_AWS_API_KEY" in env, false, `${selector}=${JSON.stringify(value)} must not pass Anthropic AWS credentials`);
				assert.equal("GOOGLE_APPLICATION_CREDENTIALS" in env, false, `${selector}=${JSON.stringify(value)} must not pass Google credentials`);
			}
		}
	});

	it("does not let security.env-allowlist bypass inactive provider-mode credential gates", () => {
		const gatedAllowlist = [...FOUNDRY_MODE_CREDENTIAL_VARS, ...ANTHROPIC_AWS_MODE_CREDENTIAL_VARS, ...AWS_MODE_CREDENTIAL_VARS, ...GOOGLE_MODE_CREDENTIAL_VARS, "MY_CUSTOM_VAR"];
		for (const step of ALL_STEPS) {
			const env = buildClaudeSeatEnv(source, step, gatedAllowlist);
			for (const name of gatedAllowlist.slice(0, -1)) assert.equal(name in env, false, `${step} must deny inactive allowlisted credential ${name}`);
			assert.equal(env.MY_CUSTOM_VAR, source.MY_CUSTOM_VAR, `${step} still forwards an ordinary allowlisted value`);
		}
	});

	it("passes each provider credential class to every role only in its matching mode", () => {
		...
		for (const selector of ["CLAUDE_CODE_USE_BEDROCK", "CLAUDE_CODE_USE_MANTLE", "CLAUDE_CODE_USE_ANTHROPIC_AWS"]) {
			for (const truthy of ["1", "true", "YES", " on "]) {
				...
				for (const step of ALL_STEPS) {
					...
					for (const name of FOUNDRY_MODE_CREDENTIAL_VARS) assert.equal(name in env, false, `${step} must still drop Foundry credentials`);
					assert.equal(env.ANTHROPIC_AWS_API_KEY, selector === "CLAUDE_CODE_USE_ANTHROPIC_AWS" ? bag.ANTHROPIC_AWS_API_KEY : undefined, `${step} ${selector} must gate Anthropic AWS API auth narrowly`);
					assert.equal("GOOGLE_APPLICATION_CREDENTIALS" in env, false, `${step} must still drop Google ADC`);
				}
			}
		}
		for (const truthy of ["1", "true", "YES", " on "]) {
			const bag = sdkShapedEnv({ CLAUDE_CODE_USE_FOUNDRY: truthy });
			for (const step of ALL_STEPS) {
				const env = buildClaudeSeatEnv(bag, step, allow);
				for (const name of FOUNDRY_MODE_CREDENTIAL_VARS) assert.equal(env[name], bag[name], `${step} Foundry=${truthy} ${name}`);
				assert.equal("ANTHROPIC_AWS_API_KEY" in env, false, `${step} must still drop Anthropic AWS auth`);
	...
	it("drops each provider credential class when its matching mode skips native auth", () => {
		const skippedModes: ReadonlyArray<{ selector: string; skipAuth: string; credentials: readonly string[] }> = [
			{ selector: "CLAUDE_CODE_USE_BEDROCK", skipAuth: "CLAUDE_CODE_SKIP_BEDROCK_AUTH", credentials: AWS_MODE_CREDENTIAL_VARS },
			{ selector: "CLAUDE_CODE_USE_MANTLE", skipAuth: "CLAUDE_CODE_SKIP_MANTLE_AUTH", credentials: AWS_MODE_CREDENTIAL_VARS },
			{ selector: "CLAUDE_CODE_USE_ANTHROPIC_AWS", skipAuth: "CLAUDE_CODE_SKIP_ANTHROPIC_AWS_AUTH", credentials: [...AWS_MODE_CREDENTIAL_VARS, ...ANTHROPIC_AWS_MODE_CREDENTIAL_VARS] },
			{ selector: "CLAUDE_CODE_USE_FOUNDRY", skipAuth: "CLAUDE_CODE_SKIP_FOUNDRY_AUTH", credentials: FOUNDRY_MODE_CREDENTIAL_VARS },
			{ selector: "CLAUDE_CODE_USE_VERTEX", skipAuth: "CLAUDE_CODE_SKIP_VERTEX_AUTH", credentials: GOOGLE_MODE_CREDENTIAL_VARS },
			{ selector: "CLAUDE_CODE_USE_ANTHROPIC_GOOGLE_CLOUD", skipAuth: "CLAUDE_CODE_SKIP_ANTHROPIC_GOOGLE_CLOUD_AUTH", credentials: GOOGLE_MODE_CREDENTIAL_VARS },
		];
		for (const { selector, skipAuth, credentials } of skippedModes) {
			const bag = sdkShapedEnv({ [selector]: "1", [skipAuth]: "true" });
			for (const step of ALL_STEPS) {
				const env = buildClaudeSeatEnv(bag, step, allow);
				for (const name of credentials) assert.equal(name in env, false, `${step} ${selector} with ${skipAuth} must drop ${name}`);
				assert.equal(env[selector], "1");
				assert.equal(env[skipAuth], "true");
			}
			const falseSkipBag = sdkShapedEnv({ [selector]: "1", [skipAuth]: "false" });
			const falseSkipEnv = buildClaudeSeatEnv(falseSkipBag, "pr-review", allow);
			for (const name of credentials) assert.equal(falseSkipEnv[name], falseSkipBag[name], `${selector} with falsy ${skipAuth} must retain ${name}`);
		}
	});

	it("drops direct Anthropic auth in every third-party provider mode even when allowlisted", () => {
		for (const selector of ["CLAUDE_CODE_USE_BEDROCK", "CLAUDE_CODE_USE_MANTLE", "CLAUDE_CODE_USE_ANTHROPIC_AWS", "CLAUDE_CODE_USE_VERTEX", "CLAUDE_CODE_USE_ANTHROPIC_GOOGLE_CLOUD", "CLAUDE_CODE_USE_FOUNDRY"]) {
			const bag = sdkShapedEnv({ [selector]: "1" });
			for (const step of ALL_STEPS) {
				const env = buildClaudeSeatEnv(bag, step, DIRECT_ANTHROPIC_CREDENTIAL_VARS);
				for (const name of DIRECT_ANTHROPIC_CREDENTIAL_VARS) assert.equal(name in env, false, `${step} ${selector} must drop stale direct auth ${name}`);
				for (const name of CLAUDE_CLI_SHARED_AUTH_VARS) assert.equal(env[name], bag[name], `${step} ${selector} must retain shared SDK auth/profile input ${name}`);
			}
		}
	});
...
describe("GitHub credential-directory masks", () => {
	...
	it("resolves relative GH_CONFIG_DIR from cwd while ignoring relative XDG candidates (#554)", () => {
		// GH_CONFIG_DIR permits a cwd-relative override; XDG basedir does not. Mask the former
		// and skip only the invalid XDG candidate while retaining the home-derived mask.
		const seatCwd = tempDir("pelaggio-gh-relative-cwd-");
		const homeRoot = tempDir("pelaggio-gh-relative-home-");
		const homeGh = join(homeRoot, ".config", "gh");
		const relativeGh = join(seatCwd, ".gh-auth");
		plantGhConfig(homeGh, "planted-home-config-token");
		plantGhConfig(relativeGh, "planted-relative-config-token");
		const invocation = buildClaudeSeatInvocation(
			{ command: "node", args: [], cwd: seatCwd },
			deniedBuildOpts({
				cwd: seatCwd,
				bwrap,
				socketPaths: [],
		...
		assert.deepEqual(
			invocation.maskedDirectories,
			[realpathSync(relativeGh), realpathSync(homeGh)].sort((a, b) => a.localeCompare(b)),
		);
	});
	...
	it("refuses to mask a GH_CONFIG_DIR that contains the active Anthropic profile store", () => {
		const explicitRoot = tempDir("pelaggio-gh-anthropic-explicit-");
		mkdirSync(join(explicitRoot, "credentials"), { recursive: true });
		plantGhConfig(explicitRoot, "planted-explicit-container-token");
		assert.throws(
			() =>
				buildClaudeSeatInvocation(
					{ command: "node", args: [], cwd },
					deniedBuildOpts({ cwd, bwrap, socketPaths: [], home: "/home/operator", xdgConfigHome: "/home/operator/.config", ghConfigDir: explicitRoot, anthropicConfigDir: explicitRoot, tmpdir: "/tmp" }),
				),
			/too wide to mask/,
		);

		const xdgRoot = tempDir("pelaggio-gh-anthropic-xdg-");
		mkdirSync(join(xdgRoot, "anthropic", "credentials"), { recursive: true });
		plantGhConfig(xdgRoot, "planted-xdg-container-token");
		assert.throws(
			() => buildClaudeSeatInvocation({ command: "node", args: [], cwd }, deniedBuildOpts({ cwd, bwrap, socketPaths: [], home: "/home/operator", xdgConfigHome: xdgRoot, ghConfigDir: xdgRoot, anthropicConfigDir: "", tmpdir: "/tmp" })),
			/too wide to mask/,
		);
	});
...
`,
	...
	it("keeps the #658 checkout extraheader residual readable to denied seats", { timeout: 15_000 }, async () => {
		// #658 residual: seat-ENV denial cannot close an on-DISK credential in the
		// bound checkout. This true-fire test must change visibly when that channel closes.
		const checkoutToken = "planted-checkout-extraheader-token-xx";
		for (const step of ["pr-review", "pr-verify"] as const) {
			const result = await runStatusForgeryProbe(step, { checkoutToken });
			assert.equal(result.hits.length, 1, `${step} can still post with the checkout-persisted token until #658`);
			assert.match(result.hits[0] ?? "", new RegExp(`Authorization: token ${checkoutToken}`));
			assert.equal(result.observed.ghStatus, 0, step);
			assert.match(result.observed.checkoutConfig, /extraheader\s*=\s*AUTHORIZATION: basic /i);
			assert.equal(result.observed.env.GH_TOKEN, null, `${step} recovered the token from disk, not env`);
		}
	});
...
async function runStatusForgeryProbe(
	step: Step,
	...
	const checkoutConfig = join(worktree, ".git", "config");
	if (options.checkoutToken !== undefined) {
		mkdirSync(dirname(checkoutConfig), { recursive: true });
		const basicCredential = Buffer.from(`x-access-token:${options.checkoutToken}`, "utf8").toString("base64");
	...
	const plantedTokens = [
		...
		...(options.checkoutToken === undefined ? [] : [options.checkoutToken]),
	];
...
function tokenFromCheckoutConfig() {
	try {
		const config = readFileSync(join(process.cwd(), ".git", "config"), "utf8");
		const match = config.match(/^\\s*extraheader\\s*=\\s*AUTHORIZATION:\\s*basic\\s+(\\S+)\\s*$/im);
		if (!match) return "";
...
const token = tokenFromEnv() || tokenFromConfig() || tokenFromCheckoutConfig();
...
process.stdout.write(JSON.stringify({
	...
	checkoutConfig: tryRead(join(process.cwd(), ".git", "config")),
	ghStatus: gh.status,
}) + "\\n");
`,
	);
