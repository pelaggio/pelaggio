...
/**
 * `pelaggio sync` — upgrade installed skills by diffing the package's
...
export function resolveConsumerRoot(cwd: string = process.cwd()): string {
	try {
		...
		throw new Error("pelaggio sync must run inside a git repository");
	}
}
...
export async function runSync(opts: SyncOptions): Promise<SyncResult> {
	...
	if (!dryRun && !force && !isTTY && !opts.prompter) {
		throw new Error("pelaggio sync: --force or --dry-run required when not running interactively");
	}
