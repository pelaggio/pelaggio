...
const claudeRunStep: RunStepFn = async (name, prompt, opts, emit) => {
	...
	const seatPreflight = await preflightClaudeSeat({ cwd: opts.cwd, step: name, envAllowlist: CONFIG.security.envAllowlist, macosSeatbeltPreview: CONFIG.claudeMacosSeatbeltPreview });
	if (!seatPreflight.ok) {
	...
	const spawnClaudeCodeProcess = (spawnOpts: SpawnOptions): SpawnedProcess => {
		...
		return spawnClaudeSeat(spawnOpts, {
			cwd: opts.cwd,
			bwrap: seatPreflight.launcher.path,
			launcher: seatPreflight.launcher,
			step: name,
