...
const claudeRunStep: RunStepFn = async (name, prompt, opts, emit) => {
	...
	const seatPreflight = preflightClaudeSeat({ cwd: opts.cwd, step: name, envAllowlist: CONFIG.security.envAllowlist, macosSeatbeltPreview: CONFIG.claudeMacosSeatbeltPreview });
	if (!seatPreflight.ok) {
