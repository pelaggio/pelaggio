...
// `pnpm pelaggio <subcommand>` — the substitution an agent reaches for when a
// CLI invocation misbehaves. `pnpm pelaggio` is the pipeline entry, not a CLI
// dispatcher, so this shape would re-enter the pipeline (TOOL-50). Subcommands
// must go through `npx pelaggio` instead.
const PNPM_AUTOPILOT_SUBCOMMAND_RE = /\bpnpm\s+pelaggio\s+(?:roadmap|worktree-deps|sync)\b/g;
...
// Digit-after-family requirement keeps worktree/branch names (`pelaggio`
...
export function lintSkillFile(absPath: string, repoRoot: string): Violation[] {
	...
	// Skills that reference pelaggio internals (scripts/pelaggio/*.ts) make no
	...
	if (fm.consumer !== false) {
		const internalRef = /(?:packages\/pelaggio\/)?scripts\/pelaggio\/[\w.-]+\.ts/.exec(body);
		if (internalRef) {
			violations.push({
				file: rel,
	...
	// TOOL-50 recursion-shaped invocations
	...
	while ((pnpmHit = PNPM_AUTOPILOT_SUBCOMMAND_RE.exec(skillBody)) !== null) {
		violations.push({
			...
			rule: "skill.pnpm-pelaggio-subcommand",
			message: "'pnpm pelaggio' is the pipeline entry; subcommands go through 'npx pelaggio' (TOOL-50)",
		});
	}
