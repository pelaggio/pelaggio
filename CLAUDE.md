...
- **Claim ledger serializes parallel picks**: `claim-ledger.ts` maintains an ephemeral, gitignored `${MAIN_REPO}/.dev/autopilot-claims.json` (a `Record<id, Claim>`) guarded by a directory-mutex lock dir (`autopilot-claims.lock`). The durable roadmap source stays the source of record; the ledger is consulted only at the pick/claim/mark-done choke points, all of which already funnel through `roadmap-cli.ts` (no adapter files change). `claim` acquires the lock → reaps stale claims → rejects with exit 3 + `claim-result: already-claimed` if an **active** claim exists (TOCTOU guard) → delegates to `roadmap.claimItem` → records. `list`/`get` overlay `status: "in-progress"` onto open items with an active claim (read-only — never takes the write lock). For the **markdown** adapter only, the lock is additionally held across `markDone`/`createItem` so their shared-file edits serialize; network adapters (github/linear) keep API calls outside the lock and lock only the JSON mutation. A claim is **stale** iff its owner `pid` is dead OR its `worktree` path is gone — reaping drops it from the overlay but leaves the `feat/<id>` branch/worktree intact for `--resume`/`/tidy`. MAIN_REPO is resolved via `git rev-parse --git-common-dir` (not `config.REPO`, which shards per-worktree); `CLAUDE_AUTOPILOT_MAIN_REPO` overrides it for tests.
- **`AUTOPILOT_OWNER_PID` carries the long-lived owner pid**: `orchestrate()` sets `process.env.AUTOPILOT_OWNER_PID ??= String(process.pid)` so the short-lived `npx roadmap claim` subprocess (a descendant the SDK spawns with inherited env) records the **orchestrator's** pid — one pid shared by all in-process parallel cycles, distinct per server-spawned `pnpm autopilot` subprocess. Liveness (`process.kill(pid, 0)`) of that pid is how reaping tells a live cycle from a crashed one. `ownerPid()` falls back to `process.ppid` (the parent `claude` process) for inline `/pick`; if a future SDK curates env, reaping degrades to worktree-existence only — acceptable.
