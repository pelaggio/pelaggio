...
- **Worktrees share MAIN_REPO's `node_modules` via symlink when lockfiles match.** `/pick`'s Claim step (and a mid-cycle guard at the top of every worktree-cwd step in `step-runner.ts`) calls `packages/autopilot/scripts/autopilot/worktree-deps.ts`: if `<worktree>/pnpm-lock.yaml` sha256 matches `<MAIN_REPO>/pnpm-lock.yaml`, it symlinks `<worktree>/node_modules → <MAIN_REPO>/node_modules` and, under the same hash gate, each `<worktree>/<pkg>/node_modules → <MAIN_REPO>/<pkg>/node_modules` for every workspace subpackage listed in the lockfile's `importers:` block (so `pnpm -r test` resolves subpackage deps correctly); on drift or missing main `node_modules`, falls through to `pnpm install --frozen-lockfile --silent` at the worktree root, which provisions every subpackage in one pass. Real (non-symlink) `node_modules` in the worktree is always left alone (user-managed); the per-subpackage `restore` branch only fires when the root just decided `restore` (coupled corruption recovery). Ownership test is `lstatSync().isSymbolicLink()`.
