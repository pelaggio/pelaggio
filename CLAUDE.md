...
- **Plan-polish block**: during the `implement` step, `step-runner` installs a PreToolUse hook (`blockPlanPolish`) that rejects Write/Edit to any path under `docs/plans/`. The implement step must EXECUTE the plan by writing code elsewhere, not edit the plan document. Only `plan` and `shakedown-plan` may write to `docs/plans/`. The review-time sibling lives in `/shakedown`'s code-review mode (issue #74): a **self-referential roadmap guard** flags (fix-now, not a hard block) any implement diff that edits the item's *own* row in `docs/roadmap-*.md`/`docs/task-index.md` — that row's lifecycle is owned by ship's mutation-lock-serialized `markDone`, so an implement-time strike-through signals premature completion and no-ops the ship-tail bookkeeping. A softer flag (not a hook) because the offending edit may arrive via a committed feature-branch commit, and it needs the item's own ID to distinguish self-bookkeeping from a legitimate `create-item` row for a *different* item.
- **PreToolUse hooks don't see Task tools** (`@anthropic-ai/claude-agent-sdk` ≥ 0.3.142): `TaskCreate` / `TaskUpdate` / `TaskGet` / `TaskList` bypass PreToolUse + PostToolUse hooks entirely. The guardrails above (worktree isolation, plan-polish) only matter for filesystem-mutating tools (Write/Edit/Bash), so the bypass is benign — but skill designers should not assume hooks can reach Task tools. If a future hook needs to govern task usage, the SDK exposes no path today.
...
- **No hardcoded model strings**: all model names live in `MODEL_PROFILES` in `config.ts`. No other file references `claude-opus-*` or `claude-sonnet-*` literals. Skill and template markdown bodies are linted for pinned IDs by `check-skills` (`model-id.hardcoded`, matching the closed `claude-(opus|sonnet|haiku|fable)-<digit>` family set across `.claude/skills/**/SKILL.md` and `.claude-templates/**/*.md`); `bump-models` is exempt wholesale because its whole purpose is documenting and refreshing those IDs.
- **PR review gate fails closed**: the `review` required status check (`.github/workflows/pr-review.yml` → `pr-review-cli.ts` → `parseReviewGate`) exits non-zero on *anything* that is not an explicit `Verdict: PASS` from a successful run — `Verdict: BLOCK`, a missing verdict, a refusal, an SDK error, max-turns, or a rate-limit park all block the merge. This is deliberately stricter than `parseVerdict` (which keeps an "engaged ⇒ APPROVE" fail-safe): a merge gate must never green on a phantom sign-off. `pr-review` is a **non-pipeline** `Step` (in `Step`/`ALL_STEPS` and every `Record<Step,T>` map, absent from `STEPS`/`PipelineStep` — same precedent as `shipwreck`), so it carries first-class budget/turns/effort/model config without affecting pipeline sequencing. See `docs/pr-review.md`.
- **Red-review closed loop is bounded to one pass** (issue #60): `.github/workflows/pr-review-revise.yml` fires on the review workflow's `workflow_run: failure` and, exactly once per PR, re-implements from the findings and re-pushes so the gate re-runs. The one-pass bound is a durable **`autopilot:revised` PR label** added *before* any work (serialized by a per-branch `concurrency` group, `cancel-in-progress: false`) — a second red review finds the label, posts a park comment, and stops; the label doubles as a manual kill switch, and `vars.AUTOPILOT_AUTO_REVISE = false` is the repo-wide off-switch. The seam is `--review-findings <path>`: a **resume-only** CLI flag (guarded in `runOrchestrator` like `--from`) that reads the file best-effort in `pipeline.ts` and injects `reviewFindingsPreamble()` into the implement step, mirroring the plan-shakedown feedback injection (an absent/unreadable file never crashes the resume). To enable CI resume, the `--no-worktree`/CI guard now accepts `--resume <id>` as an explicit-id source alongside `--item`, and the `pull-request` ship SKILL skips `gh pr create` when a PR is already open (idempotent re-ship). **Load-bearing**: the revision push must be **PAT-authed** (`actions/checkout` `token: secrets.GH_TOKEN`) — commits pushed with the default `GITHUB_TOKEN` don't trigger `pull_request` workflows, so a non-PAT push would ship but never re-review. See `docs/pr-review.md`.
- **Local revise sweep is orchestrator head-work, not a new step** (issue #76): the *active* counterpart to the disabled CI revise workflow. At the start of a pure auto-pick run (`items.length === 0`, not resume/CI/dry-run), `runOrchestrator` sweeps for red-review PRs and revises each **in-process** on the local subscription — reusing `runPipeline` with `startFrom: "implement"` + a fetched `--review-findings` file (the same in-process resume the park/auto-resume loop uses), so parking/notifications/cost/shipTarget all apply for free (no spawned subprocess). It is a hard no-op unless `revise.local` (config, **default true** — an *opt-out*) AND `roadmap.source: github-issues` AND a PR ship target (`shipIsPr`) AND `!noWorktree && !dryRun && items.length === 0`. The GitHub/git primitives live in `revise-sweep.ts` (pure, fail-soft, injected `GhRunner`/exec — any gh/git error skips, never throws into the run); orchestration stays in `pipeline.ts`. Like CI, the one-pass bound is the shared **`autopilot:revised` label added *before* any work** (`claimRevision`); labeled-still-red PRs get one idempotent `<!-- autopilot-revise-parked -->` handoff comment. Revisions do **not** consume `--cycles` (that sizes new-work throughput) but **do** count toward `--budget`, so results are pushed into `results`/`totalSpent` before the worker pool. **No new pipeline `STEP`** — no `STEPS`/`BUDGETS`/`TURN_LIMITS`/`EFFORT`/`MODEL_PROFILES` entry (deterministic head-work like ship bookkeeping / auto-resume). Parking is preserved with **zero new exit paths**: the sweep delegates to `runPipeline`'s existing `parkExit()`, pushes results pre-park-block so a parked revision auto-resumes, and `resumeOne` re-injects the gitignored `.dev/` findings file so a resumed revision still fixes the specific blockers. Only local *or* CI should be active (both race for the label); CI stays disabled repo-wide, so local is the sole reviser. See `docs/pr-review.md`.
...
- **Direct-push post-merge bookkeeping is pipeline-owned, not agent work**: for `direct-push` the agent's `ship` step ends at the merge (squash → merge into local `main` → post-merge verify → STOP, reporting `ship-merged: <id>`). Everything after the merge — recovering stray `MAIN_REPO` changes as a commit (**never discard**), `roadmap.markDone`, `roadmap.archivePlan`, the single `git push origin main`, and worktree/branch cleanup — runs in `pipeline.ts` via `runShipBookkeeping()` (`ship/bookkeeping.ts`) as **zero-turn, idempotent, best-effort** deterministic code once `verifyShipLanded()` confirms the merge. This is *not* a new pipeline `STEP` (no `STEPS`/`BUDGETS`/`TURN_LIMITS`/`EFFORT`/`MODEL_PROFILES` entry — it's deterministic tail work like `/shipwreck` recovery). The boundary is the merge because the observed failure (issue #28) was budget exhaustion *after* merging; making the pipeline own the tail means bookkeeping is guaranteed even if the agent overshoots (idempotent steps swallow double-work) or undershoots. The tail runs only on a **verified** merge — `merged && ship.ok && reportedShipMerged(ship)` — because `ship.ok` means the agent completed post-merge verification (SKILL.md step 5) and the `ship-merged: <itemId>` marker (parsed by `parseShipMerged`, matched case-insensitively to the resolved `itemId`) proves it actually reached the hand-off gate rather than ending its session successfully some other way (issue #37). A merge that landed but the agent then ran out of turns (`error_max_turns`, so verification may not have run) or hard-failed (`error`, a flagged regression) is **not** blindly pushed — it routes to `/shipwreck`, which re-runs verification with its own budget and can roll the merge back — and on a **verified recovery** (gated on `wreck.ok && reportedShipMerged(wreck) && verifyShipLanded()`, mirroring the happy-path `merged && ship.ok && reportedShipMerged(ship)`) the pipeline runs the same `runShipBookkeeping()` tail, so the tail is guaranteed on both the direct and the recovery paths (issue #30: the #28 failure mode had otherwise merely relocated to recovery). `/shipwreck` gets the same `autopilot --target=direct-push` signal `/ship` gets and stops at a hand-off gate (its SKILL.md steps 3d/6 shrink to merge-only recovery + a gate mirroring `/ship` step 5a); a shipwreck that reports `ok` but did not advance `main` (e.g. diagnosed `unknown`) is gated off by the fail-closed `verifyShipLanded()` and the cycle is reported incomplete, never pushed. (There is no consumer-agnostic verification command the deterministic tail can run itself; verification is agent-delegated via `_rubric.md`, so the tail delegates it to the agent path.) The tail is **not** unconditionally best-effort: the destructive steps (worktree/branch removal) run only after mark-done, archive, and push all genuinely succeed, so a real adapter failure (the markdown adapter no-ops an already-done item, making any throw a genuine failure), a push failure, or a `git pull` conflict (aborted via `git merge --abort`) returns `ok:false` with the branch left intact (recoverable on local `main`) and the pipeline reports the cycle incomplete rather than "shipped". `verifyShipLanded()` fails **closed** (a git error → not-landed → `/shipwreck`). The pre-ship `captureShipState()` capture fails **closed** too: a `null` result (mainRepo/worktree can't answer `git rev-parse`) makes the pipeline refuse to invoke `/ship` at all, rather than let the merge-verification block's `preShipState` guard skip silently and fall through to `target.interpretResult(ship)`'s blind `completed: ship.ok` (issue #36 — the guard shape dates to #29 but originally failed open on a null capture). A pre-ship `commitStrayBookkeeping()` guard commits any dirty `MAIN_REPO` tree before the agent merges, so the agent never has cause to discard uncommitted work. `MarkdownRoadmap.createItem`/`markDone`/`archivePlan` commit their own pathspec with `--no-verify` (mirroring each other) so a deferred `create-item` is never left unstaged for the next ship to destroy and a consumer pre-commit hook can't break the step.
- **Claims are git-native; roadmap mutations self-serialize** (issue #12): "claimed" means the `feat/<id>` branch exists — git's ref locking is the arbiter (a losing `roadmap claim` exits **3**, `pick-result: already-claimed`, recoverable). There is NO claims file, owner pid, or staleness lifecycle; release is branch deletion (ship bookkeeping / `/tidy` already own it). The markdown adapter surfaces claims as `in-progress` by scanning `feat/*` branches; github/linear surface their server-side markers (`in-progress` label / `started` state). Shared-file writers — `markDone`/`createItem`/`archivePlan` AND `commitStrayBookkeeping`'s `git add -A` sweep — take `.dev/roadmap-mutation.lock` INTERNALLY (O_EXCL token lockfile with expiry-in-content, atomic rename-verify steal/release — `roadmap/mutation-lock.ts`); callers never manage the lock. Claim worktree naming uses `WORKTREE_PREFIX` from config (env > yml > basename) in all adapters. Don't add call-site locking or a parallel claim registry.
...
`park.auto-resume`, `park.max-wait`, `revise.local`.
