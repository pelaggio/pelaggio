...
2. **Realization evidence** protects current implementation. Realization nodes retain `codeEvidence` paths and now name at least one harness-owned test observation. `ci/assurance-observations.ts` requires an exact, unskipped `node:test` pass event for every identity in the current checkout; missing, skipped, TODO, and failed observations all feed `stale-realization`. This first slice is report-only: the mutation proof that a bound test detects a hollowed mechanism is separate work, and realization claims remain non-authoritative under ADR-0027.
...
The realization-observation slice advances the preceding snapshot: the debt view now has ten checks, and every Pelaggio realization names a harness-owned test identity whose existence and current result feed `stale-realization`. `codeEvidence` remains path-existence-only; the observation binding is report-only and remains non-authoritative until the separate hollowed-mechanism mutation demonstrates that the named test is a meaningful falsifier.
