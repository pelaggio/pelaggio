...
`charter-normalization-fixtures.json` captures historical Pelaggio cases selected because later project history supplies strong counterexamples:

- **#408** — landing correctness must survive replacement of `bd merge-slot` or git CAS;
- **#176** — credentials outside the process are a mechanism for bounded authority, not the objective itself;
- **#337** — provider descriptors/abstractions are mechanisms for preserving explicit semantics across provider differences;
- **#579** — happy-path fixture cleanup fails under process death and unsafe reclamation;
- **#500** — auth/Origin/content-type are candidate controls around browser-triggered daemon authority;
- **#186** — in-toto/DSSE are rails for accountable consequence, not the desired state itself;
- **#147** — a bounded dead-link CI repair should be allowed to produce no material normalization delta.
...
Evaluate whether early normalization avoids historically known false commitments, preserves stated constraints and user intent, reduces downstream review churn, avoids needless abstraction for small work, and whether Goal materially improves durable explanations beyond proposition-only semantics.

Reject or narrow this experiment if normalization mostly restates the request, increases friction on bounded work, strips meaningful user constraints, causes independent models to invent incompatible objectives, or if a Goal layer adds no consequential answerability after normalization.
