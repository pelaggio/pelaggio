...
6. **Reconcile.** On the next run-start or post-cycle boundary, reap confirms both the forge merge and ancestry in freshly fetched `origin/main`, then marks the item done, archives its plan, and removes its clean worktree and claim branch. This includes admin merges; dirty worktrees and failed mark-done calls are retained and reported.
