...
5. **Land.** `--admin` **bypasses branch protection**, so it is an out-of-band exception, not the standard path ([ADR-0003](../decisions/0003-pr-gated-by-default.md), ADR-0005): use it **only after confirming CI is green** and the deterministic gates passed — never to merge red, and never as a habit. Wait for CI green, then `gh pr merge <N> --repo <owner/repo> --squash --admin --delete-branch`. `--admin` is required only because a local-subscription review posts as `app=None` against a branch-protection pin (in this repo, GitHub app id `15368` — environment-specific, may change) — it bypasses the *review-pin*, **not** CI. Because the admin path also skips the pipeline's post-merge tail, you must run step 6 (mark-done) and confirm post-merge verify by hand. Note there is **no deterministic guard** that stops `--admin` from merging red — on this path the supervisor *is* the guard, so a fully-automated supervisor must **programmatically confirm CI-green + the attestation** before invoking it. This is why the path is temporary. (until the review posts under the pinned app — see `roadmap-and-ship.md`.)
