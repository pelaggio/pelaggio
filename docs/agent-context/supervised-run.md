...
1. **Start from local state.** The run-start reap reconciler prunes orphaned review-head worktrees and drains previously landed PRs before the cycle begins.
...
5. **Land.** With the unpinned-`review` + `ship.target: auto-merge-pr` posture (see `docs/pr-review.md#alternative-local-review-auto-merge-unpinned-review`), landing is automatic once `ci` and `review` are green; supervision is retrospective. The admin path below applies only if the `review` app-pin is (re)enabled. `--admin` **bypasses branch protection**, so it is an out-of-band exception, not the standard path ([ADR-0003](../decisions/0003-pr-gated-by-default.md), ADR-0005). Use `npx pelaggio land --pr <N> --repo <owner/repo> --admin` rather than a raw `gh pr merge --admin`: it reads the PR's CI status directly and refuses to merge (exit 1, with a diagnostic) unless every check has completed green — deterministic, not operator discipline (issue #292). `--admin` is required only because a local-subscription review posts as `app=None` against a branch-protection pin (in this repo, GitHub app id `15368` — environment-specific, may change) — it bypasses the *review-pin*, **never** the CI-green requirement. The guard degrades to CI-green-alone until the ADR-0018 attestation (#188) lands; it never fails open. (`--admin` bypassing the review-pin is why the path is temporary — until the review posts under the pinned app; see `roadmap-and-ship.md`.)
6. **Reconcile.** On the next run-start or post-cycle boundary, reap requires the merged PR head to match the current local claim-branch tip and confirms the merge commit's ancestry in freshly fetched `origin/main`; only then does it mark the item done, archive its plan, and remove its clean worktree and claim branch. This includes admin merges; dirty worktrees, reused branch names with newer commits, and failed mark-done calls are retained and reported.
7. **Sync main if needed.** Reap intentionally does not advance local `main`. Sync it via a **rebase-pull that preserves any concurrent session's unpushed local commits** (`git pull --rebase`; never fast-forward-discard or reset).
