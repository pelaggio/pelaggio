...
1. **Safety floor (never ships on dissent).** Any finding whose harness-owned effective class resolves
   to safety in the configured ADR-0016 taxonomy (the six-token ADR table by default; extensions are
   free and contractions are owner-signed) is **Hard-block → park**, *not* Dissent, until fixed — a single
...
safety with a deterministic winner by the resolved taxonomy's safety precedence; unmatched or ambiguous evidence
...
A genuine split between successfully parsed reviewer verdicts is a deterministic, fail-closed gate for direct-push and pull-request shipping. The harness records every participating verdict and rationale in the item's per-item decision log (`docs/decision-log/<itemId>.md`), binds it to the reviewed commit and an evidence fingerprint, emits the existing best-effort `decision` notification, and parks before invoking the Judge. A matching human `proceed` resolution can release judgment-only disagreement on resume; `block`, stale or malformed evidence, and safety-class blockers remain parked. Notification failure never changes this gate.
