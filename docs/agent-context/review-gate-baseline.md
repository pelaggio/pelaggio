...
verdict parsers), so the Q14 unlinked-guarantee baseline is now **0** over the full registry rather than 6 over
the represented subset (TC-002's mechanism is the registry's own evidence command), and the
baseline is frozen so it can only shrink; realizations are 21 for 69 propositions and every
internal invariant names one; `adrMap` is derived from node `sources` (they had
