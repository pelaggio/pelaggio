...
- #647 guarantees only that managed dependency links in `packages/pelaggio/node_modules` resolve to the canonical targets derived from MAIN's `pnpm-lock.yaml` and virtual-store layout, or the cycle parks. Seat teardown and the harness path immediately before each ship-candidate ratchet run the same detect → symlink-only repair → re-verify routine under `.dev/node-modules-repair.lock`; immediately before repair it revalidates the real parent directories, parks without deleting a non-symlink in a managed link slot, and uses one non-recursive `unlink` only after `lstat` proves the entry is a symlink. The remaining race is therefore bounded to one link inode, never a tree. The routine never reads seat-authored restoration state or executes `pnpm`. #705's pre-ratchet repair is subsumed here.
- This is restoration, not isolation: a seat can still write other content in MAIN's shared `node_modules`. #262 owns the read-only bind into the execution jail, and #573 owns restore-only materialization from harness-owned inputs; findings about other shared-store content belong to their acceptance surface.
