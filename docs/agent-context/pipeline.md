...
- Authoring-review seats use the same existing dependency tree as other worktrees. Before a seat starts, the harness atomically records one `packages/pelaggio/node_modules/{diff,tsx,ulid,yaml}` snapshot shared across SHAs/processes, bound to a stable current lockfile hash and validated for existing root-`.pnpm` targets with the expected package identities. Cleanup restores missing, non-symlink, or outbound entries in its `finally` path after normal completion, provider error, or kill; a different healthy root-`.pnpm` target is left untouched.
- A skipped teardown (for example, process `SIGKILL`) is healed on the next ship harness path: immediately before the ship-candidate typecheck, `repairMainNodeModules()` restores the durable canonical snapshot under `.dev/node-modules-repair.lock`, then performs its unchanged main-scoped outbound-link repair. Healthy MAIN layouts remain a no-op and do not invoke pnpm. Private seat dependency ownership is deferred to #573 (restore-only `node_modules` ownership).
