...
- `markDone` and `archivePlan` are independent best-effort mutations after the verified merge. Their failures are accumulated as actionable warnings, without caller-level locking or discarded work, and the single push is attempted regardless. A successful push permits cleanup and completes the cycle as "shipped — bookkeeping incomplete" when metadata remains. Push failure or a `git pull` conflict (aborted via `git merge --abort`) remains fatal and leaves the branch intact, recoverable on local `main`.
