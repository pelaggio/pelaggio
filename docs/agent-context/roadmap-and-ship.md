...
`pr-review` is a non-pipeline step used by the CI merge gate. The gate (`.github/workflows/pr-review.yml` → `pr-review-cli.ts` → `pr-review-gate.ts` → `parseReviewFindings`) fails **closed**: each successful pass must emit a valid versioned, severity-tagged report. A `must-fix` finding blocks; `nice` and `note` do not. A missing or malformed report, refusal, SDK error, or max-turns also blocks. A **rate-limit** returns a third gate outcome, `gate: "park"`; in the CI runner that still maps to a red status + exit 1 (fail-closed, no wait loop on a GH Actions job), so CI behavior is unchanged. The aggregate still posts the compatibility `review` commit status, so existing branch protection and local review callers are unchanged.

Security-sensitive diffs run two independent `pr-review` sessions: the ordinary standard review and a triggered `--red-team` pass selected by guarantee-holding paths plus structured guard deltas in `classifySecurityReviewDiff()`. Either pass can block, and a triggered red-team pass that cannot complete blocks the whole required `review` check. Do not add a new pipeline `Step` for this; both sessions intentionally reuse the non-pipeline `pr-review` step key and aggregate into one PR comment / status result. This is additive seat selection, not content-based gate tiering.
...
Convergence is deterministic and bounded. Validated blocker fingerprints survive across independent iterations until a complete verifier explicitly refutes them; omission is never refutation. PASS requires a complete valid iteration with an empty carried-survivor set. Max-pass, aggregate-budget, diminishing-return, invalid-pass, verdict-split, and provider-diversity breakers all stay red. That red gate feeds the existing label-bounded revise/human handoff; the read-only review CLI does not call `parkExit()`. Rate-limit work preservation remains owned by the separate revision pipeline and its existing `parkExit()` path.

Cross-push carry (#495, local runner, `review.carry` default **off** — canary) extends that within-run contract across re-pushed SHAs: each completed local run persists its finding dispositions to `.dev/pr-review-finding-dispositions/` (digest-bound to the fleet gate record, seat-write-denied like the other evidence stores), and the next run on the same PR — after bounded git-ancestry selection of the newest **complete** bindable ancestor as narrowing watermark (refuse-on-ambiguity), with blocking-only survivor overlay from every non-watermark ancestor (incomplete or adjudication-superseded) so a retained blocker cannot be silently omitted — seeds prior survivors and narrows discovery to `watermark..head` while inspection/security/anchoring keep the full range. Consumption (seed/narrow/auto-refute) is gated on the **current** run's provider store-trust: carry consumes only when every review/verify provider in the pool has a proven store-write denial (`claude` via denial hooks, `codex` via the read-only sandbox); a pool containing a store-writable provider (`grok`, `opencode`, unknowns — default-deny) runs cold. That gate is necessary but not sufficient: it carries no producer-pool provenance, so a poisoned pre-enablement prior written by a store-writable pool stays consumable by a later all-trusted run (a #605 discard precondition), and the denial's completeness rests on #511 (harness-attested evidence closing the textual Bash-register residual, #510) — which is why it ships off. Auto-refutation of untouched-path refuted findings exists but is **dormant** under the default taxonomy (production v1 findings all classify safety-tier). Every carry-predicate failure runs cold with a diagnostic; records are written even while off. #605 covers grok-denial proof + pre-enablement record discard (and depends on #511); #613 charters the store GC. See `docs/pr-review.md` → "Cross-push carry".
