...
Cross-push carry (#495, local runner, `review.carry` default **off** — canary) extends that within-run contract across re-pushed SHAs: each completed local run persists its finding dispositions to `.dev/pr-review-finding-dispositions/` (digest-bound to the fleet gate record, seat-write-denied like the other evidence stores), and when enabled the next run on the same PR — after bounded git-ancestry prior selection with refuse-on-ambiguity and a fall-back past adjudication-superseded records — seeds prior survivors and narrows discovery to `prior..head` while inspection/security/anchoring keep the full range. Deterministic auto-refutation of untouched-path refuted findings exists but is **dormant** under the default taxonomy (production v1 findings all classify safety-tier; it activates only when findings carry judgment-resolving classification evidence). The default stays off until the store-trust prerequisite holds for every local review provider (grok's write surface at main cwd is unverified); every carry-predicate failure runs cold with a diagnostic, and records are written even while off. See `docs/pr-review.md` → "Cross-push carry".
