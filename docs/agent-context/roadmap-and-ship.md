...
Cross-push carry (#495, local runner, `review.carry` default on) extends that within-run contract across re-pushed SHAs: each completed local run persists its finding dispositions to `.dev/pr-review-finding-dispositions/` (digest-bound to the fleet gate record), and the next run on the same PR — after git-ancestry prior selection with refuse-on-ambiguity — seeds prior survivors, deterministically auto-refutes prior-refuted findings whose anchoring file the two-dot `--no-renames` interdiff left untouched (never safety-tier, under recorded tier **or** current-taxonomy resolution), and narrows discovery to `prior..head` while inspection/security/anchoring keep the full range. Every carry-predicate failure (force-push, malformed/ambiguous/unbindable record, unresolvable prior) runs cold with a diagnostic; `review.carry: false` restores per-push cold reviews while still writing records. See `docs/pr-review.md` → "Cross-push carry".
