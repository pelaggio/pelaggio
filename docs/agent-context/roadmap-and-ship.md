...
Local runner only: after a narrow hunk-local fix on a complete findings-terminal fleet review, `npx pelaggio pr-adjudicate --pr <n>` is the typed operator “go” — it does not replace full `pr-review` when the interdiff escapes the recorded repair hunks. Local adjudication accepts only harness-signed, forge-anchored fleet/source evidence; `.dev` records alone are observability.
