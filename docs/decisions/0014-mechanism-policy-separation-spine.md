...
## Context
The system's invariants are scattered across the codebase and docs — typed item-scoped effects, `STEPS` as the tabular source of truth, `ship.target` owns merge behavior, `MODEL_PROFILES` not pinned in skills, egress-broker mechanism vs. a versioned caps fixture, and (planned) `FlowPolicy` seeing a snapshot not storage. They share one organizing principle that is stated nowhere as a citable decision — and it is embodied in shipped pieces today, with the planned flow work extending it, not introducing it.
