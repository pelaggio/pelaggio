...
- **Architectural intent constrains consequential properties, not implementation topology.** Semantic intent must not prescribe repository structure merely to obtain conformance. Evidence from an implementation may challenge either its current realization or the architectural intent itself; a semantic discrepancy is a question to investigate, not authority to change code. Implementation remains evidence about the architecture, not merely a subject governed by it.
