...
- **Migration must be ratcheted, not declared complete by convention.** The graph cannot become authoritative while any channel that introduces load-bearing intent — an ADR, the trust registry, the always-loaded AGENTS.md invariant index, or an agent-context construction home — can do so without a mechanical coverage or reconciliation signal. Coverage is enumerated from the **source**, never from the graph: every ADR file, every trust-registry record, and every AGENTS.md invariant bullet must map (see Construction). Three gaps remain and are part of what promotion must close: construction homes under `docs/agent-context/` are covered only where an index bullet or ADR routes to them; anchor-substring grounding cannot detect intent strengthened or weakened around a preserved phrase; and one mapping per ADR file does not prove every load-bearing sentence in that file is represented.
...
`ci/__tests__/shadow-assurance.test.ts` — the coverage and question checks: every ADR file maps; every `trust-claims.yml` record is a public proposition whose projection status mirrors the registry (Q5); every AGENTS.md invariant bullet is covered or classified (`invariantIndex`, Q16); guarantees that name no mechanism are a shrink-only baseline (Q14); questions Q1–Q16. `ci/__tests__/assurance-views.test.ts` — the six debt diagnostics, bound to `views.json`.
