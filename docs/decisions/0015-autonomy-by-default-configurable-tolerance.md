...
2. **A machine-checkable attestation** binding those gate results ([ADR-0018](./0018-in-toto-attestation-envelope.md)) — it must assert the deterministic facts (`safety_survivors == 0`, `ci == green`, `containment == verified`), and merge fails **closed** if the attestation is absent or any asserted fact is false. Condition (2) is deliberately **not** a free-text "reasoned decision" a model can always produce: per [ADR-0014](./0014-mechanism-policy-separation-spine.md) there is no prompt-engineered lower bound, so the second condition carries deterministic teeth or it carries none. A human-readable rationale rides alongside as forensic provenance — it is not itself the withholding control.
