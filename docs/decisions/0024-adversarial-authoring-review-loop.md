...
- (−) **Revisit trigger for the diversity assumption (refined by the Review/Landing corpus audit):** reconsider the fleet if a controlled sample at equal review budget shows that one strong reviewer reproduces its material findings, or that cross-provider disagreement contributes no landed correction while adding material cost, latency, or failure surface. A sample informs the strategy rather than universally refuting provider diversity; measure through #627 before any gate change. The baseline to beat is 1.64 re-review rolls per landing (`review-gate-baseline.md`).
