...
**The ambiguity sink is non-contractible.** The class the emission classifier assigns to an *ambiguous* finding (`correctness-regression`) cannot be contracted **even by a valid owner signature** — the "ambiguous ⇒ safety" rule above is load-bearing, so demoting the sink would silently gut the floor for every unclassified finding. `resolveTaxonomy` refuses it before the signature is checked. **Scope:** the signed gate closes only the *config-only* floor shrink; the classifier, verifier, baseline table, and blocking predicates all live in the repo, so the gate is deterministic only when pelaggio runs from a pinned/installed package outside the candidate checkout (not from the PR branch under review) — the source-integrity vector is the code-review + confinement surface, not this gate.
