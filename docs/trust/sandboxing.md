...
| Claude seat PID/mount wrap | Every Claude SDK CLI child starts through a platform seat. On Linux that is Bubblewrap (`--unshare-pid`, fresh `/proc`, `--new-session`, device-capable host-root bind, `--tmpfs` on each dedicated harness-only socket parent and, for forge-denied roles, each existing GitHub CLI config directory). On macOS the default remains a closed refusal; `providers.claude.macos-seatbelt-preview: true` selects `/usr/bin/sandbox-exec` with file and unix-socket denials for those same directories plus a detached process session. Either path is fail-closed before `query()`. The child env is deny-by-default via `buildAgentEnv`; `pick`/`ship`/`shipwreck` retain forge/remote recovery credentials, and every other current `Step` does not. The detached terminal session closes the controlling-terminal command-injection path. Missing Bubblewrap, a non-Linux host without the explicit preview, or a failed zero-spend canary fails the step closed. | `TC-014`, `TC-018` |
