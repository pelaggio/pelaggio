...
Review-CONTENT integrity assumes `origin` is not rewritten by a compromised seat. The local `pr-review` resolves its diff base from the shared, seat-writable `.git/config` origin (which the confinement audit — porcelain + ref state — does not cover), so a prompt-injected seat that rewrites origin to an attacker-hosted repo presenting `main` at the real PR SHA can produce an **empty reviewed diff** and green a malicious PR. This is a review-CONTENT-integrity gap, not a credential one: the forge-WRITE denial (above), the fetch token-leak protections (the reviewed-head fetch strips ambient forge tokens, disables inherited credential helpers with `-c credential.helper=`, and refuses prompts with `GIT_TERMINAL_PROMPT=0` — `#554`), and public-sink scrubbing hold regardless. Closed by `#617` (host-brokered review fetch — the harness prepares the reviewed checkout from a trusted remote; the seat never fetches). Bounded in practice: it requires a targeted prompt-injection under **autopilot** review of **untrusted external PRs with auto-merge armed**; it does not affect supervised first-party review.
