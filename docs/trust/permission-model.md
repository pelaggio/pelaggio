...
The reviewed-head fetch no longer takes its destination from the shared, seat-writable `.git/config` origin (which the confinement audit — porcelain + ref state — does not cover). The destination is rebuilt from trusted config: the path from the configured `ghRepo`, and the host from the operator's configured host unless the origin's own host is already trusted (`#554`). A seat that rewrites origin therefore cannot move the fetch, so it can neither disclose an ambient credential to an attacker host (SSH agent, askpass, `~/.ssh`, an inherited `GIT_CONFIG_PARAMETERS` extraheader — none of which the earlier four-token strip covered) nor substitute the `main` the diff is taken against. `url.<base>.insteadOf` in the seat-writable repo config is resolved offline before the fetch and refused when it leaves the trusted host. Scheme follows the operator's setup, so an ssh-remote repo authenticated by `gh auth login` keeps working — the refusal is scoped to the untrusted destination, not to the work. Auth still rides only the one child's host-scoped extraheader, with ambient forge tokens stripped, inherited credential helpers disabled (`-c credential.helper=`), and prompts refused (`GIT_TERMINAL_PROMPT=0`).

The residual `#617` closes is now narrower and structural rather than destination-selection: the harness still fetches into, and diffs inside, a checkout the seat can write, so integrity rests on detection and on ordering rather than on the seat being unable to touch the reviewed tree at all. `#617` (host-brokered review fetch — the harness prepares a read-only reviewed checkout from a trusted remote and hands it to a seat that never fetches) remains the durable fix. Bounded in practice: exploiting what remains requires a targeted prompt-injection under **autopilot** review of **untrusted external PRs with auto-merge armed**; it does not affect supervised first-party review.
