...
| Worktrees | Sibling worktree paths chosen by roadmap adapter/prefix | Item branch checkout, edits, test output, local diffs. | Remove after preserving needed work. The audit always gates siblings and, by default, main; dirty-main mode applies provider-specific main protection. | `TC-011`, `TC-015` |
