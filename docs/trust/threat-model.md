...
| Item worktree | Mutating workspace | Write/edit hooks, worktree dependency guard, and a shipped post-step confinement audit that fails the step on any change to the main checkout or a sibling worktree; not an OS sandbox, so writes outside any tracked git root are out of scope. | `TC-011`, `TC-015` |
...
| Tampering | Injected instructions write outside the item worktree or corrupt the main checkout. | Worktree hooks block direct main-repo write/edit paths, the install guard blocks worktree-side package installs, and a shipped post-step confinement audit fails the step on any committed or uncommitted change to the main checkout or a sibling worktree, catching sibling/relative/`$HOME`/`cd`/symlink shell escapes. Writes to paths outside any tracked git root are not caught by this audit (not an OS sandbox) and are bounded by tool/egress scoping. | `TC-011`, `TC-015` |
...
Pelaggio currently bounds prompt-injection blast radius but does not detect or neutralize injection as a first-class defense (`TC-015`). The agent can still run broad tools inside the worktree; the worktree boundary is now enforced by a shipped post-step confinement audit that fails the step on any change to the main checkout or a sibling worktree (`TC-011`), but this is not an OS sandbox, so writes to paths outside any tracked git root remain bounded only by tool/egress scoping (`TC-015`). A compromised or unsuitable model provider remains in the operator's trust base (`TC-006`). Child processes inherit the parent environment, and verbose logs are not scrubbed today (`TC-014`). Auto-merge relies on external branch protection rather than in-code verification (`TC-013`).
