...
Note the gap between *written* and *checked*: receipts are collision-guarded and content-bound, and `verifyExecutionReceipt` has **zero production callers** — evidence written and never verified. **Retracted (#483):** an earlier revision claimed the join was already broken before destruction. That was an instrument artifact — `p3-dossier.ts` resolved worktree-relative receipt paths (`types.ts:67`) under the main repo, so `present: false` was guaranteed; corrected, the receipts are present. F5 stands as originally written: the evidence is destroyed with the worktree at ship, not dangling beforehand. **Measured and standing:** "what caused this lifecycle transition" is a new unanswerable — the ship-authored PR body that closed the issue lived in `.dev/ship/pr-body-483.md`, now deleted, surviving only as mutable GitHub state. Per §1.5 K6 the write happens at emission, including for failed and superseded attempts.
