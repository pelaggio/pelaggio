...
**F2 — Production lifecycle gap (merged→done is *torn*, not unowned).** `runShipBookkeeping` invoked exactly once, gated `target.name === "direct-push"` (`pipeline.ts:1946,1972`); production target is `auto-merge-pr`; no `Closes #N` templating anywhere in `scripts/` or `.claude/skills/`. **Measured on #484's merge (2026-08-11):** one second after merge the issue *closed* — GitHub honoured a `Closes #483.` line the **ship agent chose to write into the PR body** (`closingIssuesReferences: [#483]`), not a harness mechanism. Every deterministic transition still failed to fire: `in-progress` stayed, the local **and remote** `feat/<id>` claim branches persisted (blocking re-pick with exit 3), the worktree remained with all four execution receipts. The state is worse than a stall — the tracker reads done while a live claim branch keeps the item ineligible, and whether it happens at all depends on prose the model may not emit next time. It is also a live inversion of the mechanism/policy spine: a model-authored artifact performed a roadmap state transition on the production path. The de-facto reconciler is the stale-scan `shipped-by-commit` heuristic feeding quarantine, then an operator `stale-resolve --as done`. The rigor is inverted relative to the configured path.

**F3 — `ShipTarget` too thin.** Interface = `buildPrompt` + `interpretResult` only; 9 `target.name === "direct-push"` branches in `pipeline.ts` (1755, 1823, 1832, 1837, 1846, 1859, 1867, 1946, 2250), plus the related `shipIsPr` split at 2817 carry pre-ship capture, verification, retry, shipwreck recovery, and the tail. `auto-merge-pr.ts` is a 30-line near-clone of `pull-request.ts`. Also: the documented git CAS fence for direct-push (**`--force-with-lease=main:<sha>`**) does not exist — `pushMain` is a plain push with pull-retry whose unverified-origin-merge risk is self-acknowledged (`bookkeeping.ts:52–63`); the only true fence is `gh pr merge --match-head-commit` (`land-cli.ts:81`). G1 (#464) is the open work to build the real fence.
...
1. **Widen `ShipTarget`** to `{buildPrompt, interpretResult, preShip, verify, tail}`; move the 10 direct-push branches and the bookkeeping invocation behind it; merge `pull-request.ts`/`auto-merge-pr.ts`. Close F2 minimally **without** a per-target tail: extract `runShipBookkeeping`'s body into one `reconcileLanded(item, evidence)` closer with two triggers — observed local merge (direct-push, in-cycle) and observed forge merge (PR targets, from the drain). A `tail` on `ShipTarget` is homogeneous with the interface only for direct-push and invites a second implementation for the PR path; §K1 settles on one closer and this is where it starts. `ShipTarget` therefore widens to `{buildPrompt, interpretResult, preShip, verify}` only. The closer must also **normalize or suppress model-authored `Closes #N` prose at ship time** — measured on #484, that line closed the issue ahead of any reconciler and would race the closer (F2).
...
**K6 — Evidence is written durable at emission.** *(Sequencing, measured: K6 is a precondition for K1, not a parallel track. On #483's merge the worktree-local evidence — 4 execution receipts, 1 review record, 6 flow-event segments, the plan — **survived**, because the merged→done gap also leaves the worktree standing; the dossier is byte-identical before and after merge. Worktree destruction exists only on `direct-push`. The moment the closer starts reconciling and cleaning up, it begins destroying provenance that currently survives by accident, so write-at-emission ships first or the closer ships with cleanup deferred.)* Compact records (receipts, review records, gate records) go to the durable evidence home at write time — not promoted at ship — with age/count retention. Failed and superseded attempts retain their evidence; this is a debugging requirement ("the only way to debug reliably") and the precondition for the F-hypothesis probe to test the real design rather than a strawman. Fat transcript retention remains out of scope (the G-hypothesis boundary).
