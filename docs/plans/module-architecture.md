...
| L4 `orchestration` | *Sequences steps or seats end-to-end?* | `pipeline.ts`, `helpers.ts` (current; deleted in step 4), `cycle-support.ts` (new, step 4), `orchestrator.ts`, `cycle-result.ts` (new, step 5; `runPipeline` stays in `pipeline.ts`), `steps/<step>.ts` (new, step 9), `pr-review-gate.ts` (new, step 2) |
...
| 5 | **Split `pipeline.ts`** (pure move, no signature change): `orchestrator.ts` (`runOrchestrator`, `orchestrate`, park/reset waiting, hermetic defaults, `OrchestratorDeps`), `cycle-result.ts` (`resultIcon/Status/Detail`). `runPipeline` **stays in `pipeline.ts`** — a trust claim (`docs/trust/trust-claims.yml:128`) and three shadow-graph nodes cite that path as evidence, so renaming it to `cycle.ts` would be churn for its own sake. `main.ts`, `index.ts`, `revise-cli.ts` import the orchestrator directly. | `pipeline.ts`, `orchestrator.ts`, `cycle-result.ts`, 3 entry files → `orchestrator.test.ts` (already separate) imports move | orchestrator and cycle in separate files; `pipeline.ts` ≤ 2,600 lines until step 9 shrinks it | 4 |
...
| 7b | **Shared atomic writer.** `record-store.ts` (L1) exports `writeAtomically(path, bytes)` (one tmp+rename implementation, one temp-name convention) and `readJsonOr(path, validate)`. It is a *byte* primitive, **not** an envelope: each register keeps its own on-disk shape, its own version field name (`SessionRecord.version`, `StaleQuarantineFile.version` are persisted contracts and are not renamed), its own validator and failure convention (`throw` vs `null`), and its own key semantics (gate records overwrite; receipts reject). Migrated in one PR (each writer is a two-line change): freshness-gate, pr-review-gate, adjudication-sources, dispositions, review-records, doc-review-records, doc-review-transcripts, review-requests, effects (which was not atomic before), stale-quarantine. **Excluded, and documented as such in `record-store.ts`**: `execution-receipt` (byte-identical idempotence, collision-on-different-bytes, and post-write verify are its guarantee — it keeps its own writer), `attempt-identity` (O_EXCL create-only allocator), `file-lock` (a lock, not a record), `flow-events` (append-only JSONL), and `confinement/sessions` (its writer is probe-injected — `confinement-sessions.test.ts` drives it through an in-memory `writeFile`/`rename`/`mkdir` — so routing it through the shared writer would delete that seam; amended at implementation, #731). | one module + its test per PR | one tmp+rename implementation for plain records; the four exclusions documented in `registers.ts`; on-disk bytes of every migrated register unchanged (golden fixtures) | 7a |
...
| 9 | **Steps as functions — one step per PR.** `steps/<step>.ts` exports `run<Step>(input: <Step>Input, deps): Promise<<Step>Outcome>` where `<Step>Input` lists **only** the closure bindings that block reads today (e.g. implement reads `assignment, available, worktree, profile, flags, roadmap, cost`). No hooks, no registry, no shared context bag; `Step`-keyed tables hold config, never behaviour. Extract in closure-dependency order: `shakedown-code` (last block, fewest downstream readers) → `implement` → `shakedown-plan` → `plan` → then the pick prologue and ship epilogue. Each PR carries its slice of `pipeline.test.ts` into `steps/__tests__/<step>.test.ts`. Config-map collapse is **dropped**: `resolveStepSettings` already is the single step seam and the storage is profile-first by design. | `pipeline.ts`, `steps/*` → `pipeline.test.ts` slices | `pipeline.ts` < 600 lines, contains no step body; each `steps/<step>.ts` has its own test file | 5, 6 |
