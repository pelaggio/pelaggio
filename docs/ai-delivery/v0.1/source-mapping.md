...
The Statement subject is outside the predicate. `CycleProvenance.git.headSha` is not automatically the landed merge-commit SHA-256: the future emitter must resolve the final artifact and apply its documented Git hashing procedure. `EffectsManifest` data can become an `effects-manifest` evidence entry only if captured before `dispatchStepEffects()` deletes the successful manifest; its digest proves the recorded orchestration intent, not completion of every external effect.
