...
| **C** | Split. C1 (harness-owned resolution) holds; C2 (the state resolved over is not agent-writable) is target state and was **measured violated** — a ship-agent-authored `Closes #N` performed a roadmap transition | P5 |
...
**A and I** need no amendment on this evidence. **H was never probed before P5** — its earlier
